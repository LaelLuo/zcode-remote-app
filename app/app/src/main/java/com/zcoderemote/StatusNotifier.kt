 ...
 * 无 TERMINAL 态：链接判死=瞬时回配置界面（清凭据停服务），不驻留在通知上。
 */
...
object StatusNotifier {
    ...
    // 完成提醒（完成时岛收起且无任何提示，需要独立可感知）：常驻渠道是 LOW 无声，
     ...
     *  RECONNECTING 也上岛：断网/重载期间岛显示「重连中」，比岛消失更可感知。
     ...
     *  titleOverride：通知标题（会话视图=会话名、列表视图=「任务列表」，
    ...
    fun update(
        ...
        // 「进入 DONE」≠「完成事件」——浏览一个早已完成的会话也会进入 DONE，
        // 真事件=同一实体从跑变停/从跑变等，由 MainActivity 的
    ...
    fun buildNotification(ctx: Context, state: SessionState): Notification {
        ...
        // 版本门：core 1.17 的 Compat 在 API<36 会把 ProgressStyle 降级为传统进度条（字节码
        // 实证），不是忽略——为保持旧设备通知形态与旧版完全一致，仅 API 36+ 设置上岛三件套
        if (state in promotedStates) {
            // 上岛=裸提升请求 + 胶囊短文本（不用 ProgressStyle：进度样式对无进度数值的会话是装饰；提升请求与短文本在旧系统仅为
